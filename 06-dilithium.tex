...
\subsection{Removing the Query Bound: the Deterministic Scheme} \label{sec-dil-derand}
...
+\sum_{i=1}^{4}\AdvPPRF{\PRF_i}{\advD^{\mathsf{pprf}}_{j,\beta,i}}
...
+ \sum_{i=3}^{4}\AdvPPRF{\PRF_i}{\advD^{\mathsf{pprf}}_{\mathsf{trig},i}}
...
The chain specializes the local two-record hybrid of Appendix~\ref{app-derand} to a slot known before key generation, with the lattice substitutions. Replace the honest circuit by the two-point circuit that hardwires
$$
\bigl\{(\enc{\mat{w}_j, m_j}''', c_j),
(\enc{\mat{W}_{m_j}, m_j}''', c_t(m_j))\bigr\}
$$
and punctures $k_1, k_2$ at the two points and $k_3, k_4$ at $m_j$. The circuits compute the same function: the trigger comparison at slot $m_j$ succeeds only at $\mat{W}_{m_j}$, which $\mathsf{Bad}_{\mathsf{eq}}$ separates from $\mat{w}_j$, and puncturing preserves functionality elsewhere. Thus iO bounds the change.
...
\paragraph*{Outlook: the adaptive port.} Section~\ref{sec-dil-derand} completes the static half of the deterministic-signing port. What remains from Part~I is the adaptive result, and we record the plan.

Under the matrix-LWE profile of Proposition~\ref{prop-tlf-lwe}, its LWE-based
TLF supplies the required message-digest layer, while the DDH cascade of
Proposition~\ref{prop-tlf-ddh} gives the alternative polynomial-image profile
under classical assumptions. Constructing the TLF is therefore not the
remaining obstacle to an adaptive lattice result. Such a theorem would still
require the lattice-specific proof steps below, and it
would not yet cover ML-DSA itself because module structure, $B_\tau$,
compression, and hints remain untreated; the hedged default mode also stays
open.
...
\adam{All: Remaining items are compression and hints; a concrete conversion function and evidence for cSIS/cMSIS over $B_\tau$; the module lift for $B_\tau$; and the adaptive deterministic port. Suggested split: Ojaswi+Carter on parameters, Xinyu on $B_\tau$, and Alex+Adam on the adaptive port and module lift.}
