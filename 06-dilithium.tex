...
$$
\AdvCSIS{\GDec,\chal,q,n,m',\ell,\gamma,\eta}{\advA}
\le
4 \cdot \AdvSIS{q,n,m'+\ell+n,\max(\gamma,1,\eta)}{\advB} \;.
$$
\end{lemma}
...
$$
so $\advB$ returns $(\hat{\mat{z}}, \mat{c}, \hat{\mat{u}})$. Its entries are bounded by $\max(\gamma, 1, \eta)$, and it is nonzero because $(\hat{\mat{z}}, \mat{c}) \ne \mat{0}$. Thus $\advB$ wins whenever $\mat{C}$ is invertible and $\advA$ wins. The instance $\advA$ receives is uniform for every invertible $\mat{C}$, so the two events are independent, and the claimed inequality follows from the invertibility probability.
\end{proof}
...
$$
which is a $\CSIS$ solution $(\mat{z}_1{+}\mat{v}_1,\ \mat{z}_2{+}\mat{v}_2,\ \mat{w}')$ with norms at most $\Bclip + B$, nontrivial because $c \ne 0^\ell$. The verifier's slack $\mat{z}_2$ and the mask $\mat{v}_2$ end up in the error coordinate of the solution; this is the reason $\CSIS$ has that coordinate. The reduction generates the PRF keys itself, so it can recompute $(\mat{v}_1, \mat{v}_2)$ from the forgery.
...
Assume the setting and hypotheses of Theorem~\ref{thm-dil}, with the $\CSIS$ hypothesis replaced by $\SIS_{q,n,\colsA+\ell+n,\Bclip+B}$. Then the scheme $\DSScheme[\HF^{\mathsf{lio}}]$ is EUF-static secure.
\end{corollary}
...
\end{aligned}
$$
where the single iO term is the hardwiring switch of $\Gm_3$; the later hops change only the distribution of the hardwired data, which their own reductions build and obfuscate themselves, so no further iO invocation occurs. Every term is negligible under the hypotheses.
\end{proof}
