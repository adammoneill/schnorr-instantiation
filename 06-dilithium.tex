...
The pre-hash is available. The proof guesses the digest of a query from the enumerated lossy image, and the enumeration only has to run in time polynomial in $\secp$ and $r$. At quasi-polynomial $r$ the Peikert--Waters construction with a rounded output gives such a tunable lossy function under subexponentially hard LWE; Section~\ref{sec-conclusion} sketches the construction and records what remains to verify. With it the adaptive port is post-quantum up to the assumptions underlying iO. Instantiated instead with the DDH-based ELF cascade, the port would still give the first standard-model adaptive result for a lattice Fiat--Shamir scheme, under classical assumptions, which is worth having on its own. Deterministic ML-DSA is the signing mode such a theorem covers; the hedged default mode stays open, as it does for BIP340 on the Schnorr side.
