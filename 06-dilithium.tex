...
The condition $\mat{0} \notin \chal$ lets verification skip a zero-challenge check and keeps every forgery nontrivial. The bound $\nu$ replaces $\ell$ in the response bound $\Bclip = B - \nu$, in the requirement $B \ge 4\nu(\colsA+n)$, in the shift bounds of Lemma~\ref{lem-dil-sim}, and in the final cSIS solution norms, which become $\Bclip + B$ as before; $\omega$ records the coefficient size of the challenge. The reductions do not need $f$ to map a uniform input to a uniform challenge. In the key-recovery proof the simulator samples from the actual enumerated hash image. In the static proofs, after the LWE hop one may retain the natural distribution $f(\mat{U})$ for uniform $\mat{U}\in\Zq^n$; the rejection-sampling change of variables is exact for any challenge distribution independent of the nonce, and the trigger branch samples the same distribution. Thus $\beta$ is useful for assessing the plausibility of the circular assumption, but it causes no loss in these proofs. With $\Fq_q$ replaced by $f$ also in Construction~\ref{constr-dil-tlf}, the scheme, the hash constructions, and the three theorems generalize to any such conversion function, with $\CSIS_f$ over the chosen $\chal$ as the hypothesis of the static theorems. The gadget reduction to SIS remains specific to $\Fq_q$. \adam{All: The generic proof claim has now been re-traced using the natural challenge distribution $f(\mat{U})$ rather than replacing it by uniform on $\chal$. It still wants an independent pass before being stated as a formal theorem.}
...
\adam{All: Remaining items are compression and hints; a concrete conversion function and evidence for cSIS/cMSIS over $B_\tau$; the module lift for $B_\tau$; and the derandomized adaptive port. Suggested split: Ojaswi+Carter on key recovery and parameters, Xinyu on $B_\tau$, and Alex+Adam on the adaptive port and module lift.}
