...
Key generation ignores $X$. Lossy keys are used only in the proof. The outer hash maps the possibly structured range $\Ygrave$ to the scalar required by Schnorr. Collision resistance rules out a constant outer map; it is not used in the key-recovery reduction.
...
$$
Game $\Gm_0$ is the BK-CMA game. Game $\Gm_1$ changes the tunable lossy function to lossy mode at the same value $r$. Game $\Gm_2$ enumerates the lossy image once and replaces honest signing by rejection sampling.
...
Fix $x\in\Zp$, $X=g^x$, a key $K$, an outer-hash key $k$, a message $m$, and a list $S$ such that $\Image(\TLFEv(K,\cdot))\subseteq S$. Let
$$
C=\{\CRHFEv(k,y):y\in S\}.
$$
For $(c,s)\in C\times\Zp$, call $(c,s)$ good if
$$
\CRHFEv\bigl(k,\TLFEv(K,\enc{g^sX^{-c},m})\bigr)=c.
$$
...
For $R\in\GG$, define
$$
c_R:=\CRHFEv(k,\TLFEv(K,\enc{R,m}))
\qquad\text{and}\qquad
s_R:=\mathsf{dlog}_g(R)+c_Rx\pmod p.
$$
The enumeration condition gives $c_R\in C$. We also have
$$
g^{s_R}X^{-c_R}=R,
$$
...
\paragraph*{Why the construction uses two modes.} The scheme deploys the injective mode, and the lossy mode occurs only in the proof. One might ask whether the second mode is needed at all, since the rejection-sampling oracle of $\Gm_2$ works for any hash function with a small enumerable image. If the deployed hash were itself lossy, the simulation would apply directly to the real game, and key-recovery security would follow from hardness of DL against time $r\cdot\poly(\secp)$ with no assumption about modes.

The reason is that the simulator is also an attack. Lemma~\ref{lem-sim} says each trial of the rejection-sampling oracle produces a valid signature with probability exactly $1/|C|$, and the trial uses only the verification key. An adversary that runs the same loop on a message of its choice therefore obtains a signature on it. This gives the following bound, which holds for every hash function and uses nothing about how it is built.

\begin{proposition} \label{prop-lossy-forge}
Let $H \colon \GG \times \bits^{\msglen} \to \Zp$ be efficient with $|\Image(H)| \le r$, and suppose a set $C \supseteq \Image(H)$ of size at most $r$ can be computed in time $\tau$. Then there is an adversary that runs in time $\tau + \secp r \poly(\secp)$, makes no signing queries, and wins the EUF-CMA game against $\SchScheme[\GG,H]$ with probability at least $1 - e^{-\secp}$.
\end{proposition}
...
Two consequences. First, deploying a lossy hash would give a scheme that is universally forgeable in time about $r$. Taking the quasi-polynomial $r = 2^{(\log \secp)^2}$ at $\secp = 128$ gives $r = 2^{49}$, so the scheme would offer about $49$ bits of unforgeability security in a group chosen for $128$. The injective mode has no such attack, and this is what the second mode buys: the conclusion is about a scheme that is not already broken, and mode indistinguishability is what carries it there from the lossy game.

Second, and independently of our construction, Proposition~\ref{prop-lossy-forge} shows that lossiness alone cannot give unforgeability. Any image small enough to rejection-sample against is small enough to forge against, in the same time. So the technique of this section is confined to key recovery for a structural reason, and unforgeability needs a different tool. That is the subject of Section~\ref{sec-io-uf}, where the conversion function of the CDL assumption has small preimages and hence a large image, which is exactly the regime in which the loop above is too slow to be either a simulation or an attack.
...
Paillier and Vergnaud~\cite{AC:PaiVer05} prove BK-CMA security for Schnorr with any hash function under OMDL. Their result covers \emph{any} hash function, while ours covers a particular keyed family. Our group assumption is ordinary DL, but the hash construction additionally uses exponential DDH and collision resistance. OMDL is interactive; all of our assumptions are non-interactive.
