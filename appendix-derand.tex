...
Throughout, $d = \dig(K,m)$ denotes the digest of $m$ under the pre-hash key $K$, and the slot of a query on $m$ is $d$. Fix a PPT adversary $\advA$ making at most $q_s$ signing queries. In a given execution, let $N\le q_s$ be the number of distinct queried messages, and write $m_j$ for the $j$-th message when it first appears, for $j\in[N]$. Signing is deterministic, so a repeated query is answered by replaying the earlier answer in every game below, and the hybrid runs over $m_1,\ldots,m_N$.
...
\heading{Uniform nonces.} Replace each honest nonce $r_m = \PRFEv_0(k_0,m)$ by an independent uniform value of $\Zp$, once and for all. Equivalently, before key generation presample
$$
r_j\getsr\Zp\qquad\text{for }j\in[q_s],
$$
assign $r_j$ to the $j$-th new message $m_j$ when it appears, and reuse it when that message is repeated. The key $k_0$ appears nowhere else, so a single distinguisher bounds the change by $\AdvPRF{\PRF_0}{\advD_0}$. Honest nonce points are now uniform, as they are for a randomized signer.
...
\heading{Changing distinct message $j$.} Fix $j$ and restrict to executions with $N \ge j$. At each endpoint, introduce the event
$$
...
For $b\in\{0,1\}$, independently sample $v_b,s_b\getsr\Zp$ and set $R_b=g^{s_b}X^{-v_b}$ and $T_b=(R_b,v_b,s_b)$. Condition the joint distribution of $(T_0,T_1)$ on $R_0\ne R_1$. Let $V$ be any random variable that depends on $(T_0,T_1)$ only through the unordered set $\{(R_0,v_0),(R_1,v_1)\}$ and through data whose distribution is unchanged when $T_0$ and $T_1$ are exchanged. Then $(V,R_0,s_0)$ and $(V,R_1,s_1)$ are identically distributed.
\end{lemma}

\begin{proof}
Before conditioning, the joint distribution of $(T_0,T_1)$ is invariant under exchanging the two triples. The event $R_0\ne R_1$ is also invariant under that exchange, so the conditioned joint distribution remains invariant, although the two triples need no longer be independent. Equivalently, sample the unordered pair $\{T_0,T_1\}$ and use an independent uniform bit $\beta$ to choose its ordering. By hypothesis $V$ is a function of the unordered pair and of swap-invariant data, so its distribution does not depend on $\beta$. Hence $(V,R_\beta,s_\beta)$ and $(V,R_{1-\beta},s_{1-\beta})$ have the same distribution, which is the claim.
\end{proof}
