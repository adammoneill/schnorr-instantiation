...
Fix a PPT adversary $\advA$ making at most $q_s=q_s(\secp)$ signing queries, and let $N\le q_s$ be the number it makes in a given execution. Since $q_s$ is polynomial and $r$ is super-polynomial, $q_s\le r$ for all sufficiently large $\secp$. Throughout, $d=\dig(m)$ denotes the encoded digest $\enc{\TLFEv(K,m)}_d$ of a message $m$ under the current pre-hash key $K$.
...
The reduction now guesses the digest of query $j$. At key generation it computes $S\getsr\TLFEnum(\mathsl{td})$, removes duplicates, encodes the entries, and extends the resulting list to length exactly $r$ with distinct strings with first bit 1, which are never encodings of range elements. Call the result $L_S$. The reduction chooses an index $\widehat\imath \getsr [r]$, sets $\widehat d \gets L_S[\widehat\imath\,]$, and aborts at the end unless $d_j = \widehat d$, where $d_j = \dig(m_j)$. Except with probability $\nu_{\mathsf{enum}}(\secp,r)$, the list $S$ contains every digest the lossy key can produce, so $d_j$ occurs in $L_S$, at exactly one position. The index $\widehat\imath$ is uniform and independent of everything else, so on the executions where $d_j$ occurs in $L_S$ the guess is correct with probability exactly $1/r$, independently of the execution. Writing $E_b := [N \ge j \land \Gm_b \Rightarrow 1]$ for $b\in\{j-1,j\}$,
$$
...
\heading{A trigger-branch forgery.} Let $E_{\mathsf{trig}}$ be the event that the forgery is valid and takes the trigger branch. The reduction builds the padded list $L_S$ as in the signing hops, chooses an index $\widehat\imath \getsr [r]$ and a salt $\widehat{\salt} \getsr \bits^{\saltlen}$, and sets $\widehat u = (L_S[\widehat\imath\,], \widehat{\salt})$ before publishing the verification key. It aborts unless $\widehat u=(\dig(m^*),\salt^*)$. Except with probability $\nu_{\mathsf{enum}}(\secp,r)$, the digest $\dig(m^*)$ occurs in $L_S$, and then the guess is correct with probability exactly $1/(r\cdot2^{\saltlen})$, independently of the execution, so the case costs the factor $r\cdot2^{\saltlen}$.
