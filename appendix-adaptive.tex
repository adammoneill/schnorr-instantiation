...
If $\widehat s=\PRFEv_4(k_4,\widehat t)$ and $Z=g^{\widehat s}$, then $C_{\mathsf{forg}}$ and $\CircS$ compute the same function, so iO permits the switch. Puncturable-PRF security then replaces $\widehat s$ by a uniform value, and the circuit uses $Z=g^{\widehat s}$.
...
\authnote{Adam}{Constant check for your pass: each $\epsilon_j$ has $2$ iO and $8$ PRF terms (forward and reverse sandwich); the aggregation constant $16$ above is loose and just needs $r^2 2^{\saltlen}\eps$ absorption; $q\le r$ uses $r$ super-polynomial.}
