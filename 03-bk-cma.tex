...
$s=\rho_R+H(R,m)x\bmod p$, exactly as in honest Schnorr signing.
\end{proof}

The lemma assumes nothing about $H$ beyond the containment. It does not assume
that the values of $H$ are uniform on $C$, that they are spread evenly, or that
every element of $C$ is a value of $H$. The reason is that the count runs over
$R$ rather than over $c$. Each $R$ has exactly one pair $(c,s)$ that produces it
and passes the test, so there are exactly $p$ such pairs among the $|C|p$ pairs a
trial can draw. A value $c$ with many preimages passes often and a value with
none never passes, and the two effects cancel in the sum
$\sum_{c\in C}|\{R:H(R,m)=c\}|=p$. Only the containment matters, and a $C$ larger
than the set of values $H$ actually takes costs trials rather than correctness.

Repeating the trial is what makes it useful. Trials are independent and each
...
answers every adaptive signing query using Lemma~\ref{lem-sim}. The answers have exactly the
...
the distribution in $\Gm_1$ unless its rejection loop fails, which for
$\secp|C|$ trials happens with probability at most $e^{-\secp}$. Hence
$$
...
trial succeeds with probability $1/|C|$, independently, so all $\secp|C|$ trials
fail with probability at most $(1-1/|C|)^{\secp|C|}\le e^{-\secp}$. Any output is a valid signature on $m^*$, and
