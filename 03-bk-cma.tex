...
\subsection{The Simulation} \label{sec-bk-sampling}

The simulation rests on a counting fact that has nothing to do with Schnorr.

\begin{lemma} \label{lem-sim}
Let $A$, $B$, and $C$ be finite nonempty sets, let $\varphi_c\colon B\to A$ be a
bijection for each $c\in C$, and let $H\colon A\to C$. Consider one trial that
samples $c\getsr C$ and $b\getsr B$, sets $a\gets\varphi_c(b)$, and accepts
exactly when $H(a)=c$. The trial accepts with probability $1/|C|$, and
conditioned on acceptance $a$ is uniform on $A$.
\end{lemma}

\begin{proof}
Fix $a\in A$. A trial produces $a$ and accepts if and only if $c=H(a)$ and
$b=\varphi_{H(a)}^{-1}(a)$, and that trial does accept. So accepting trials
correspond one to one with the elements of $A$, and there are $|A|$ of them
among the $|C||B|=|C||A|$ equally likely trials. The acceptance probability is
therefore $1/|C|$, and conditioned on acceptance the accepted $a$ is uniform on
$A$.
\end{proof}

The lemma assumes nothing about $H$. It does not assume that the values of $H$
are uniform on $C$, that they are spread evenly, or that every element of $C$ is
a value of $H$. The count runs over $A$ rather than over $C$: a value $c$ with
many preimages is accepted often and a value with none is never accepted, and
the two effects cancel in the sum $\sum_{c\in C}|H^{-1}(c)|=|A|$. A larger $C$
costs trials rather than correctness.

\paragraph*{Applying it to Schnorr.} \label{par-bk-apply}
Fix $x\in\Zp$ and $X=g^x$, let $H(\cdot,m)$ be the challenge function for the
message being signed, and let $C\subseteq\Zp$ be any finite set with
$$
\{H(R,m):R\in\GG\}\subseteq C .
$$
Take $A=\GG$ and $B=\Zp$, and for $c\in C$ let $\varphi_c(s)=g^sX^{-c}$, which
is a bijection from $\Zp$ to $\GG$. The containment makes $H(\cdot,m)$ a
function from $A$ to $C$, so Lemma~\ref{lem-sim} applies: the trial that samples
$c\getsr C$ and $s\getsr\Zp$, sets $R\gets g^sX^{-c}$, and accepts when
$H(R,m)=c$ accepts with probability $1/|C|$ and yields a uniform $R$.

The accepted pair is an honest signature. Acceptance gives $g^s=RX^{c}$ with
$c=H(R,m)$, so $s=\rho+cx$ where $R=g^{\rho}$. That is what the signer computes
on nonce $R$, and $R$ is uniform, so the accepted $(R,s)$ has exactly the
distribution of an honest Schnorr signature on $m$.

Repeating the trial is what makes this useful. Trials are independent and each
accepts with probability $1/|C|$, so $N$ of them all fail with probability
$(1-1/|C|)^{N}\le e^{-N/|C|}$; taking $N=\secp|C|$ gives $e^{-\secp}$.
...
answers every adaptive signing query by the trial above. The answers have
...
domain. So the trial of Section~\ref{sec-bk-sampling} applies, and each signing
query has exactly the distribution in $\Gm_1$ unless its rejection loop fails,
which for $\secp|C|$ trials happens with probability at most $e^{-\secp}$. Hence
$$
...
$H(R,m)=\HFEv(\hk,(R,m))$ and the set $C$. By Lemma~\ref{lem-sim}, applied as in
Section~\ref{sec-bk-sampling} with this $C$, each trial succeeds with
probability $1/|C|$, independently, so all $\secp|C|$ trials fail with
probability at most $(1-1/|C|)^{\secp|C|}\le e^{-\secp}$, and any output is a
valid signature on $m^*$. The adversary made no signing query.
\end{proof}
